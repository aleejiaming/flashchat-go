...
sequenceDiagram
    ...
    actor UserA as 使用者 A
    participant Nginx as Nginx (負載平衡器)
    participant SrvA as Server A (Port 8081)
    participant Redis as Redis (中繼站 / Pub-Sub)
    participant DB as PostgreSQL (資料庫)
    participant SrvB as Server B (Port 8082)
